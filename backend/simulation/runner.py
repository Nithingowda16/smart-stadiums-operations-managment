...
def execute_single_step(db: Session):
    ...
    latest_telemetry_cache = {
        ...
        "match": match_dict,
        "crowd": decisions["raw_subsystems"]["crowd"],
        "sustainability": decisions["raw_subsystems"]["sustainability"],
        "active_incidents": emergency_list,
        "gate_queues": crowd_list
    }
...
async def run_simulation_loop():
    ...
    while True:
        try:
            ...
            telemetry = execute_single_step(db)
            db.close()
            
            if broadcast_callback:
                await broadcast_callback(telemetry)
                
        except Exception as e:
            print(f"FIFA ONE AI: Error in simulation step: {e}")
